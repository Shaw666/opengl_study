...
void OpenGLPlayer::RenderFrame(uint8_t *data, uint32_t width, uint32_t height)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, width, render_height_,
                 0, GL_RED, GL_UNSIGNED_BYTE, data);
    glUniform1i(uniform_y_, 0);

    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, texture_u_);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, width / 2,
